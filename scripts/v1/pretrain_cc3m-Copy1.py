...
#   --unfreeze_epoch 22 \
#   --gene_lr_ratio 20 --gene_warmup_steps 1000 \
...
def train_one_epoch(
    ...
    for batch in train_loader:
        ...
        # —— 关键：每个 iteration 都根据 global_step(optimizer step) 决定是否冻结 gene
        if args.model == "ours":
        ...
        if not torch.isfinite(loss).all():
            if is_main_process():
                print("[NaN] loss is not finite!")
            raise RuntimeError("NaN/Inf loss detected, stop to avoid corrupting checkpoints.")
        ...
        if bad:
            ...
            if is_main_process():
                with torch.no_grad():
                    ...
                    sc = scaler.get_scale() if args.amp else None
                    print(f"[BAD FEAT] step={global_step} img_ok={img_ok} txt_ok={txt_ok} "
                          f"img_absmax={img_absmax:.3e} txt_absmax={txt_absmax:.3e} "
                          f"logit_scale(exp)={ls:.3f} scaler_scale={sc}")

                # 额外检查：是不是模型参数已经坏了（如果参数 NaN，就不是“偶发坏样本”，而是训练发散）
                bad_param = None
            ...
            # ---- 同步跳过：必须清梯度 + 重置累积 ----
            optimizer.zero_grad(set_to_none=True)
            micro_step = 0  # 重置累积，避免用“缺失梯度”的 accum 做 step

        # ✅ backward（这一步是 scaler 需要的！）
        if args.amp:
        ...
        # —— 到这里才是一个 optimizer step —— #

        # ✅ AMP: unscale 必须在 backward 之后、clip 之前、step 之前
        if args.amp:
            scaler.unscale_(optimizer)

        # grad clip（在 unscaled grad 上做）
        if args.grad_clip > 0:
            torch.nn.utils.clip_grad_norm_(base_model.parameters(), args.grad_clip)

        # unfreeze window grad sanity check（AMP 下不要 raise）
        if unfreeze_step <= global_step < unfreeze_step + 50:
            ...
            if bad and is_main_process():
                print("[BAD GRAD] first non-finite UN-SCALED grad param:", bad)
        ...
        if args.amp:
            ...
            scaler.step(optimizer)    # overflow 会自动 skip
            scaler.update()
