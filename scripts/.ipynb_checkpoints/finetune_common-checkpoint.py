...
class FinetunedModelAdapter:
    ...
    def encode_image(self, images):
        ...
        with torch.cuda.amp.autocast(enabled=self.amp, dtype=self.amp_dtype):
            z = self.model.encode_image(images) if hasattr(self.model, "encode_image") else self.model(images)
        z = z.float()
    ...
    def encode_text(self, tokens):
        ...
        with torch.cuda.amp.autocast(enabled=self.amp, dtype=self.amp_dtype):
            z = self.model.encode_text(tokens) if hasattr(self.model, "encode_text") else self.model(tokens)
        z = z.float()
        return torch.nn.functional.normalize(z, dim=-1)
