...
# Flickr30k (Karpathy json) ITM dataset
# -----------------------------
class KarpathyMatchingDataset(torch.utils.data.Dataset):
    """Binary ITM dataset from Karpathy-style JSON.

    Produces (PIL.Image, caption:str, label:int) where label=1 means matched pair.
    Negatives are generated on-the-fly by pairing the same image with a caption from a different image.

    Expected Karpathy JSON:
      {
        "images": [
          {
            "split": "train"/"val"/"test",
            "filename": "xxx.jpg",
            "filepath": "optional/subdir",
            "sentences": [{"raw": "..."} | {"tokens": [...]} | {"sentence": "..."} ...]
          }, ...
        ]
      }
    """
    def __init__(
        ...
        self.karpathy_json = karpathy_json
        self.split = split
        self.pos_ratio = float(pos_ratio)
        random.seed(int(seed))

        data = read_json(karpathy_json)
        images = data.get("images", data)  # robust

        # filter by split
        items = []
        for im in images:
            sp = im.get("split", None)
            if sp is None:
                # if no split field, accept everything (caller can slice via max_samples)
                items.append(im)
            elif str(sp).lower() == str(split).lower():
                items.append(im)

        # build image paths + caption pairs
        self.img_paths = []
        self.pairs = []  # (img_idx, caption)
        for im in items:
            fn = im.get("filename") or im.get("file_name") or im.get("img") or im.get("image") or ""
            fp = im.get("filepath") or ""
            # some karpathy json store filepath like "train2014"; for flickr it is often empty
            if fp:
                img_path = os.path.join(img_root, fp, fn)
            else:
                img_path = os.path.join(img_root, fn)
            img_idx = len(self.img_paths)
            ...
            for s in sents:
                if isinstance(s, str):
                    ...
                    cap = s.get("raw") or s.get("sentence")
                    if cap is None and "tokens" in s:
                        toks = s.get("tokens") or []
                        cap = " ".join(toks)
                if cap is None:
                    continue
                self.pairs.append((img_idx, str(cap)))

        if len(self.pairs) == 0:
            raise ValueError(f"No (image, caption) pairs found in {karpathy_json} for split='{split}'")

        if max_samples is not None and int(max_samples) > 0:
            self.pairs = self.pairs[: int(max_samples)]

    def __len__(self):
        return len(self.pairs)

    def __getitem__(self, index: int):
        img_idx, caption = self.pairs[index]
        if random.random() < self.pos_ratio:
            ...
            while self.pairs[j][0] == img_idx:
                j = random.randrange(len(self.pairs))
            cap_use = self.pairs[j][1]

        image = pil_loader(self.img_paths[img_use])
        return image, cap_use, label
...
def build_flickr_loader(args, tokenize, transform, split: str = "val"):
    """Build Flickr30k ITM loader (Karpathy json) for cross-dataset generalization eval."""
    if (not args.flickr_images) or (not args.flickr_karpathy_json):
        raise ValueError("Flickr loader requested but --flickr_images/--flickr_karpathy_json not provided.")

    ds = KarpathyMatchingDataset(
        img_root=args.flickr_images,
        karpathy_json=args.flickr_karpathy_json,
        split=split,
        pos_ratio=args.pos_ratio,
        seed=args.seed,
        max_samples=args.flickr_max,
    )
...
@torch.no_grad()
def evaluate(args, model, loader, amp_dtype: torch.dtype) -> Tuple[float, float, float, float, float, float]:
...
def train_one_epoch(
    ...
    for it, (images, tokens, labels) in enumerate(loader):
        ...
        if is_main_process() and (global_step % args.diag_every == 0):
            ...
            if getattr(args, "report_throughput", 1) == 1:
                ...
                tp = (steps * args.batch_size * ws) / dt
                tp_str = f" throughput={tp:.1f} samp/s"
                last_tp_t = now
                last_tp_step = global_step
    ...
    acc = correct / max(1, total)
    return global_step, float(avg_loss), float(acc), float(tp_epoch)
...
def main():
    ...
    parser.add_argument("--flickr_when_best", action="store_true",
                        help="If set: only run Flickr evaluation when COCO val improves (saves time).")
    parser.add_argument("--flickr_max", type=int, default=-1,
                        help="Cap Flickr30k samples (pairs) for quick sanity checks; -1 means full.")
    ...
    # efficiency
    parser.add_argument("--report_throughput", type=int, default=1, choices=[0, 1],
                        help="Print throughput (samples/sec) for train/eval.")
    ...
    # Flickr cross-dataset eval availability
    args._has_flickr = (args.eval_flickr == 1) and bool(args.flickr_images) and bool(args.flickr_karpathy_json)
    ...
    if args.eval_only:
        ...
        if val_loader is not None:
            v_loss, v_acc05, v_auc, v_thr, v_best_acc, v_tp = evaluate(args, model, val_loader, amp_dtype=amp_dtype)
            if is_main_process():
                print(
                    f"[VAL] loss={v_loss:.4f} acc@0.5={v_acc05:.3f} auc={v_auc:.3f} best_thr={v_thr:.3f} best_acc={v_best_acc:.3f} tp={v_tp:.1f} samp/s"
                )
            results.update(
                {
                    "val_loss": v_loss,
        ...
        if test_loader is not None:
            t_loss, t_acc05, t_auc, t_thr, t_best_acc, t_tp = evaluate(args, model, test_loader, amp_dtype=amp_dtype)
            if is_main_process():
                print(
                    f"[TEST] loss={t_loss:.4f} acc@0.5={t_acc05:.3f} auc={t_auc:.3f} best_thr={t_thr:.3f} best_acc={t_best_acc:.3f} tp={t_tp:.1f} samp/s"
                )
            results.update(
                {
                    "test_loss": t_loss,
    ...
    # Flickr30k cross-dataset loaders (Karpathy json)
    flickr_loaders = {}
    if args._has_flickr:
        transform_flickr = build_clip_image_transform(args.image_size, is_train=False)
        for sp in args.flickr_eval_splits:
            try:
                _, fl = build_flickr_loader(args, tokenize, transform_flickr, split=sp)
                flickr_loaders[sp] = fl
                if is_main_process():
                    print(f"[FLICKR] built split={sp} pairs={len(fl.dataset)}")
            except Exception as e:
                if is_main_process():
                    print(f"[FLICKR][WARN] failed to build split={sp}: {e}")
    ...
    for epoch in range(args.epochs):
        global_step, train_loss, train_acc, train_tp = train_one_epoch(
            args,
            model,
            train_loader,
            train_sampler,
        ...
        if is_main_process():
            print(f"[E{epoch:03d}] train_loss={train_loss:.4f} train_acc@0.5={train_acc:.3f} tp={train_tp:.1f} samp/s")

        do_val = (val_loader is not None) and ((epoch + 1) % args.val_every == 0) and is_main_process()
        if do_val:
            val_loss, val_acc05, val_auc, best_thr, best_acc, val_tp = evaluate(args, model, val_loader, amp_dtype=amp_dtype)
            print(
                f"[E{epoch:03d}] VAL loss={val_loss:.4f} acc@0.5={val_acc05:.3f} "
                f"auc={val_auc:.3f} best_thr={best_thr:.3f} best_acc={best_acc:.3f} tp={val_tp:.1f} samp/s"
            )
            ...
            # Flickr30k cross-dataset evaluation (generalization)
            flickr_metrics = None
            run_flickr = (args._has_flickr and (improved if args.flickr_when_best else True))
            if run_flickr and len(flickr_loaders) > 0:
                flickr_metrics = {}
                for sp, fl in flickr_loaders.items():
                    f_loss, f_acc05, f_auc, f_thr, f_best_acc, f_tp = evaluate(args, model, fl, amp_dtype=amp_dtype)
                    if is_main_process():
                        print(
                            f"[E{epoch:03d}] FLICKR-{sp.upper()} loss={f_loss:.4f} acc@0.5={f_acc05:.3f} "
                            f"auc={f_auc:.3f} best_thr={f_thr:.3f} best_acc={f_best_acc:.3f} tp={f_tp:.1f} samp/s"
                        )
                    flickr_metrics[sp] = {
                        "loss": f_loss,
                        "acc05": f_acc05,
            ...
            if improved:
                ...
                best_record = {
                    ...
                    "val_throughput": val_tp,
                    "train_throughput": train_tp,
                }
                if test_metrics is not None:
                    best_record.update(test_metrics)

                if flickr_metrics is not None:
                    best_record["flickr"] = flickr_metrics
    ...
    if (val_loader is None) and (args.eval_test == 1) and (test_loader is not None) and is_main_process():
        ...
        print(
            ...
            f"auc={t_auc:.3f} best_thr={t_thr:.3f} best_acc={t_best_acc:.3f} tp={t_tp:.1f} samp/s"
        )
